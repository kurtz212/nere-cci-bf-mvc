        ...
        <div style={{ display:"flex", gap:"10px" }}>
          ...
          <button onClick={onClose}
            style={{ flex:1, padding:"12px", borderRadius:"10px", background:"#fff",
              border:"1.5px solid #E2EDE6", color:"#6B9A7A", fontWeight:600,
...
export default function Admin() {
  ...
  const chargerUsers = useCallback(async () => {
    ...
    } catch { console.warn("Erreur chargement users"); }
    ...
    try {
      ...
      if (data.success) {
        setActivites(data.data.map(log => ({
          id:    log.id || log._id,
          texte: log.description || `Recherche — ${log.resultatCount || 0} résultat(s)`,
          heure: new Date(log.createdAt).toLocaleString("fr-FR",
            { day:"2-digit", month:"short", year:"numeric", hour:"2-digit", minute:"2-digit" }),
        })));
      } else {
        setActivitesErreur(data.message || "Erreur.");
      }
    } catch { setActivitesErreur("Serveur inaccessible."); }
  ...
  }, [section, adminUser, chargerPubs, chargerUsers, chargerActivites]);
  ...
  const sauvegarderUtilisateur = async () => {
    ...
    try {
      ...
      const data = await res.json();
      if (!data.success) {
        setUserError(data.message || "Impossible de créer.");
      } else {
        await chargerUsers();
        setShowUserForm(false);
        setFormUser({ nom:"", prenom:"", email:"", telephone:"",
          fonction:"Gestionnaire", typeCompte:"administration",
          role:"manager", password:"", isActive:true });
      }
  ...
  const confirmerSuspension = async (duree, raison) => {
    ...
    if (duree !== "indef") {
      ...
      now.setHours(now.getHours() + (map[duree] || 24));
  ...
  const S = {
    wrap:    { display:"flex", minHeight:"100vh", fontFamily:"Arial, Helvetica, sans-serif", background:"#F5FAF7" },
    side:    { width:sidebarOpen?"240px":"64px", background:"#ffffff", flexShrink:0, display:"flex",
               ...
               position:"sticky", top:0, height:"100vh", boxShadow:"4px 0 16px rgba(0,0,0,0.06)" },
    ...
    textarea:{ width:"100%", padding:"10px 14px", borderRadius:"10px", border:"1.5px solid #E2EDE6",
               ...
               color:"#1A2E1F", resize:"vertical", minHeight:"160px" },
  };
  ...
  const statutBadge = (s) => ({
    "publie":"#4DC97A", "publié":"#4DC97A", "brouillon":"#D4A830",
    "actif":"#4DC97A", "suspendu":"#FF6B6B",
    "subscriber":"#4DC97A", "visitor":"#D4A830", "manager":"#4A9EFF", "admin":"#b8860b",
  }[s] || "#999");
  ...
  const renderDashboard = () => (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start",
        ...
        <div>
          <h2 style={{ fontSize:"22px", color:"#00904C", margin:0 }}>Vue d'ensemble</h2>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:"16px", marginBottom:"24px" }}>
        {kpis.map(k => (
          <div key={k.label} style={{ ...S.card, display:"flex", alignItems:"center", gap:"14px" }}>
            <div>
              <div style={{ fontSize:"28px", fontWeight:900, color:k.couleur }}>{k.val}</div>
      ...
      <div style={S.card}>
        <div style={{ fontWeight:700, fontSize:"15px", color:"#0A3D1F", marginBottom:"16px" }}>
          Activités récentes
        </div>
        {activitesLoading ? (
          <div style={{ color:"#9a866b", fontSize:"13px" }}> Chargement...</div>
        ) : activitesErreur ? (
  ...
  const renderPublications = () => (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between",
        ...
        <div>
          <h2 style={{ fontSize:"22px", color:"#0A3D1F", margin:0 }}>Publications</h2>
          <p style={{ color:"#6B9A7A", fontSize:"13px", marginTop:"4px" }}>
            {pubs.length} publication{pubs.length>1?"s":""} ·{" "}
            {pubs.filter(p => /^publi/i.test(p.statut)).length} publiée{pubs.filter(p => /^publi/i.test(p.statut)).length>1?"s":""}
          </p>
        </div>
        <div style={{ display:"flex", gap:"10px" }}>
  ...
  const renderUtilisateurs = () => (
    <div>
      <div style={{ display:"flex", justifyContent:"space-between",
        ...
        <div>
          <h2 style={{ fontSize:"22px", color:"#0A3D1F", margin:0 }}>Utilisateurs</h2>
          <p style={{ color:"#6B9A7A", fontSize:"13px", marginTop:"4px" }}>
            ...
            {[
              ...
              <div key={f.k}>
                ...
                <input style={S.input} type={f.t} value={formUser[f.k]}
                  onChange={e => setFormUser(p => ({...p, [f.k]:e.target.value}))}
                  placeholder={f.p}/>
              </div>
            ))}
          </div>
              ...
              <tr key={u.id} style={{ borderBottom:"1px solid #E2EDE6",
                ...
                <td style={{ padding:"12px 14px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
                    ...
                    <div>
                      <div style={{ fontWeight:700, color:"#0A3D1F", fontSize:"13px" }}>
                        {u.prenom} {u.nom}
                      </div>
                      {u.statut==="suspendu" && u.suspendJusquau && (
                        <div style={{ fontSize:"10px", color:"#CC6600", marginTop:"2px" }}>
                          Jusqu'au {new Date(u.suspendJusquau).toLocaleDateString("fr-FR",
      ...
      <div style={{ marginTop:"16px", padding:"12px 16px", background:"#F0F8F3",
        ...
        fontSize:"12px", color:"#6B9A7A" }}>
         La suppression de comptes est désactivée pour des raisons de traçabilité. Utilisez la suspension pour restreindre l'accès.
      </div>
    </div>
  );
  ...
  const renderActivites = () => (
    <div>
      <div style={{ marginBottom:"24px" }}>
        <h2 style={{ fontSize:"22px", color:"#0A3D1F", margin:0 }}>Journal d'activités</h2>
        <p style={{ color:"#6B9A7A", fontSize:"13px", marginTop:"4px" }}>
      ...
      <div style={S.card}>
        {activitesLoading ? (
          <div style={{ padding:"18px", color:"#6B9A7A" }}> Chargement...</div>
        ) : activitesErreur ? (
    ...
    <div style={S.wrap}>
      ...
      <aside style={S.side}>
        <div style={{ padding:sidebarOpen?"20px 20px 16px":"20px 12px 16px",
          ...
          {sidebarOpen && (
            <div>
              <div style={{ fontSize:"17px", fontWeight:900, color:"#060606" }}>
                NERE <span style={{ color:"#4DC97A" }}>Admin</span>
              </div>
              <div style={{ fontSize:"10px", color:"#6B9A7A", marginTop:"2px" }}>CCI-BF · Backoffice</div>
            </div>
          )}
          <button onClick={() => setSidebar(o => !o)}
            style={{ background:"rgba(0,144,76,0.08)", border:"none", borderRadius:"8px",
              width:"30px", height:"30px", color:"#00904C", cursor:"pointer", fontSize:"14px", flexShrink:0 }}>
            {sidebarOpen ? "◀" : "▶"}
          </button>
        </div>
        ...
        <div style={{ padding:"8px", borderTop:"1px solid #E2EDE6" }}>
          <button onClick={() => navigate("/Chatadmin")}
            style={{ width:"100%", display:"flex", alignItems:"center",
              gap:sidebarOpen?"10px":"0", justifyContent:sidebarOpen?"flex-start":"center",
              padding:sidebarOpen?"10px 12px":"10px", borderRadius:"10px", border:"none",
              cursor:"pointer", background:"#00904C", color:"#fff",
              fontWeight:700, fontSize:"13px", fontFamily:"inherit", marginBottom:"6px" }}>
            {sidebarOpen && <span> Messagerie</span>}
          </button>
