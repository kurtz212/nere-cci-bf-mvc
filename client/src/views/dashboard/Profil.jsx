...
const NAVBAR_CSS = `
  * { font-family: Arial, Helvetica, sans-serif !important; }
  ...
  .nere-navbar-profil .nav-pill .nav-btn:hover { color:#fff; background:rgba(255,255,255,0.12); }
  .nere-navbar-profil .nav-pill .nav-btn.active { color:#0A3D1F; background:#4DC97A; font-weight:700; box-shadow:0 2px 8px rgba(77,201,122,0.4); }
  .nere-navbar-profil .u-chip { display:flex; align-items:center; gap:8px; padding:5px 12px 5px 5px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); border-radius:100px; cursor:pointer; color:#fff; font-size:13px; font-weight:600; flex-shrink:0; }
  ...
  .nere-navbar-profil .u-avatar { width:30px; height:30px; border-radius:50%; background:#4DC97A; color:#0A3D1F; display:flex; align-items:center; justify-content:center; font-weight:800; font-size:12px; flex-shrink:0; }
  .nere-navbar-profil .u-chip.active .u-avatar { background:#0A3D1F; color:#4DC97A; }
  .nere-dropdown-profil { position:absolute; z-index:9999; top:calc(100% + 10px); right:0; background:#fff; border-radius:16px; border:1px solid #E2EDE6; min-width:220px; overflow:hidden; box-shadow:0 16px 48px rgba(0,0,0,0.14); }
  .nere-dropdown-profil .dd-head { padding:14px 18px 10px; border-bottom:1px solid #F0F4F1; background:linear-gradient(135deg,#F5FAF7,#fff); }
  ...
  .nere-dropdown-profil .dd-role  { display:inline-flex; align-items:center; gap:5px; margin-top:6px; background:#E8F5EE; color:#00904C; border-radius:100px; padding:3px 10px; font-size:10px; font-weight:700; text-transform:uppercase; }
  .nere-dropdown-profil .dd-item { padding:10px 18px; font-size:13px; color:#0A3D1F; cursor:pointer; }
...
export default function Profil() {
  ...
  // ── Historique abonnement (recharges + déductions) ──
  const [showHistoAbo, setShowHistoAbo]       = useState(false);
  const [histoAbo, setHistoAbo]               = useState([]);
  const [histoAboLoading, setHistoAboLoading] = useState(false);
  const [histoAboErreur, setHistoAboErreur]   = useState("");
  ...
  const estPrivilegie = (u) => u?.role === "admin" || u?.role === "manager";
  ...
  // ── Charger l'historique d'abonnement ──
  // Essaie plusieurs routes selon ce qui existe dans l'API,
  // puis reconstruit l'historique depuis les demandes si rien ne fonctionne.
  const chargerHistoAbo = useCallback(async () => {
    const token = localStorage.getItem("token");
    if (!token) return;
    setHistoAboLoading(true); setHistoAboErreur("");

    // Routes à essayer dans l'ordre
    const routes = [
      `${API}/abonnements/historique`,
      `${API}/abonnements/transactions`,
      `${API}/abonnements/mon-historique`,
    ...
    for (const route of routes) {
      try {
        ...
        if (data.success && Array.isArray(data.data) && data.data.length >= 0) {
          setHistoAbo(data.data);
          setHistoAboLoading(false);
          return;
        }
      } catch { continue; }
    }

    // Fallback : construire l'historique depuis les demandes existantes
    try {
      const resDemandes = await fetch(`${API}/demandes/mes-demandes`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const dataDemandes = await resDemandes.json();

      if (dataDemandes.success && Array.isArray(dataDemandes.data)) {
        // Construire un historique approximatif à partir des demandes débitées
        const lignesDemandes = dataDemandes.data
          .filter(d => d.montantEstime && d.montantEstime > 0)
          .map(d => ({
            _id:         d._id,
            type:        "deduction",
            montant:     -d.montantEstime,
            description: `Requête — ${d.typeRequete || "Demande"}`,
            createdAt:   d.createdAt,
            soldeApres:  undefined,
          }));

        setHistoAbo(lignesDemandes);
        if (lignesDemandes.length === 0) {
          setHistoAboErreur(""); // Pas d'erreur, juste vide
        }
      } else {
        setHistoAboErreur("Aucune donnée d'historique disponible.");
      }
    } catch {
      setHistoAboErreur("La route /abonnements/historique n'existe pas encore dans votre API. Créez-la ou consultez la documentation backend.");
    }

    setHistoAboLoading(false);
  }, []);

  const toggleHistoAbo = () => {
    if (!showHistoAbo && histoAbo.length === 0) chargerHistoAbo();
    setShowHistoAbo(o => !o);
  };
  ...
  const changerMdp = async () => {
    if (!mdpForm.ancien||!mdpForm.nouveau||!mdpForm.confirm) { setMdpMsg({ texte:"Tous les champs sont obligatoires.", type:"erreur" }); return; }
    if (mdpForm.nouveau !== mdpForm.confirm) { setMdpMsg({ texte:"Les nouveaux mots de passe ne correspondent pas.", type:"erreur" }); return; }
    if (mdpForm.nouveau.length < 6) { setMdpMsg({ texte:"Minimum 6 caractères requis.", type:"erreur" }); return; }
  ...
  // Calcul du total rechargé dans l'historique
  const totalRecharge = histoAbo
    .filter(h => h.type === "recharge" || h.montant > 0)
    .reduce((s, h) => s + (h.montant || 0), 0);

  const totalDepense = histoAbo
    .filter(h => h.type === "deduction" || h.montant < 0)
    .reduce((s, h) => s + Math.abs(h.montant || 0), 0);

  const Chip = ({ label, color }) => (
    <span style={{ background:color?`${color}15`:"var(--green-pale)", color:color||"var(--green-dark)", border:`1px solid ${color?`${color}33`:"rgba(34,160,82,0.2)"}`, borderRadius:"100px", padding:"2px 10px", fontSize:"11px", fontWeight:600 }}>
      {label}
    </span>
  );
  ...
  const InputMdp = ({ name, label, placeholder }) => (
    <div className="profil-field">
      ...
      <div style={{ position:"relative" }}>
        <input className="profil-input" type={showMdpVis[name]?"text":"password"}
      ...
      <div style={{ position:"relative", zIndex:1 }}>
        ...
        <nav className="nere-navbar-profil">
          ...
          <div className="nav-pill">
            {NAV_LINKS.map(link => (
              <button key={link.key} className="nav-btn" onClick={()=>navigate(link.path)}>{link.label}</button>
            ))}
          </div>
        ...
        <div className="profil-layout">
          ...
          <aside className="profil-sidebar">
            ...
            <nav className="profil-menu">
              {onglets.map(item=>(
                <div key={item.key} className={`profil-menu-item ${activeTab===item.key?"active":""}`} onClick={()=>setActiveTab(item.key)}>
                  <span>{item.label}</span>
                </div>
              ))}
              <div className="profil-menu-item danger" onClick={handleLogout}><span> Déconnexion</span></div>
            </nav>
          </aside>
                ...
                <div className="profil-section-header">
                  ...
                  {!editing
                    ...
                    : <div style={{ display:"flex", gap:"10px" }}>
                        ...
                        <button className="btn-save" onClick={handleSave} disabled={saving}>{saving?"":"Enregistrer"}</button>
                      </div>
                  }
                </div>
              ...
              <div className="profil-section">
                ...
                {abonnement && abonnement.solde<=0 && (
                  <div style={{ background:"#FFE5E5", border:"2px solid #FF6B6B", borderRadius:"10px", padding:"16px", marginBottom:"20px", display:"flex", alignItems:"center", gap:"12px" }}>
                    ...
                    <div><div style={{ fontWeight:700, color:"#FF3333" }}>Crédit épuisé</div><div style={{ fontSize:"13px", color:"#D32F2F", marginTop:"4px" }}>Veuillez recharger pour continuer à utiliser le service</div></div>
                  </div>
                )}
                {abonnement && abonnement.solde>0 && abonnement.solde<1000 && (
                  <div style={{ background:"#FFF4E5", border:"2px solid #D4A830", borderRadius:"10px", padding:"16px", marginBottom:"20px", display:"flex", alignItems:"center", gap:"12px" }}>
                    <span style={{ fontSize:"24px" }}></span>
                    <div><div style={{ fontWeight:700, color:"#D4A830" }}>Crédit faible</div><div style={{ fontSize:"13px", color:"#B8860B", marginTop:"4px" }}>Moins de 1 000 FCFA — Pensez à recharger</div></div>
                  </div>
                )}
                ...
                <div style={{ display:"flex", gap:"12px", marginBottom:"24px" }}>
                  <button onClick={()=>navigate("/formules")}
                    style={{ flex:1, background:"#00904C", color:"#fff", border:"none", borderRadius:"10px", padding:"13px 16px", cursor:"pointer", fontSize:"14px", fontWeight:700 }}>
                     Ajouter du crédit
                  </button>
                  {/* ── Bouton Voir l'historique — toggle ── */}
                  <button onClick={toggleHistoAbo}
                    style={{ flex:1, background:showHistoAbo?"#00904C":"#F0F4F0", color:showHistoAbo?"#fff":"#333", border:showHistoAbo?"none":"none", borderRadius:"10px", padding:"13px 16px", cursor:"pointer", fontSize:"14px", fontWeight:600 }}>
                    {showHistoAbo ? "▲ Masquer l'historique" : " Voir l'historique"}
                  </button>
                </div>

                {/* ══ HISTORIQUE ABONNEMENT — affiché si showHistoAbo ══ */}
                {showHistoAbo && (
                  <div style={{ marginBottom:"24px" }}>
                    {/* En-tête avec résumé */}
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:"16px" }}>
                      <div style={{ fontWeight:700, fontSize:"16px", color:"#0A2410" }}>
                         Historique des transactions
                      </div>
                      <button onClick={chargerHistoAbo} disabled={histoAboLoading}
                        style={{ padding:"6px 12px", background:"#E8F5EE", border:"none", color:"#00904C", borderRadius:"7px", fontSize:"12px", fontWeight:600, cursor:"pointer" }}>
                         Actualiser
                      </button>
                    </div>
                    ...
                    {histoAbo.length > 0 && (
                      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:"12px", marginBottom:"16px" }}>
                        <div style={{ background:"#E8F5EE", borderRadius:"12px", padding:"16px", textAlign:"center", border:"1px solid rgba(0,144,76,0.15)" }}>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:"6px" }}>Total rechargé</div>
                          <div style={{ fontSize:"22px", fontWeight:900, color:"#00904C" }}>{totalRecharge.toLocaleString("fr-FR")}</div>
                          <div style={{ fontSize:"11px", color:"#6B9A7A", marginTop:"2px" }}>FCFA</div>
                        </div>
                        <div style={{ background:"#FFF0F0", borderRadius:"12px", padding:"16px", textAlign:"center", border:"1px solid rgba(232,85,85,0.15)" }}>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:"6px" }}>Total dépensé</div>
                          <div style={{ fontSize:"22px", fontWeight:900, color:"#E85555" }}>{totalDepense.toLocaleString("fr-FR")}</div>
                          <div style={{ fontSize:"11px", color:"#6B9A7A", marginTop:"2px" }}>FCFA</div>
                        </div>
                        <div style={{ background:"#F5FAF7", borderRadius:"12px", padding:"16px", textAlign:"center", border:"1px solid rgba(0,144,76,0.1)" }}>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", marginBottom:"6px" }}>Solde actuel</div>
                          <div style={{ fontSize:"22px", fontWeight:900, color:abonnement?.solde>0?"#00904C":"#E85555" }}>{(abonnement?.solde||0).toLocaleString("fr-FR")}</div>
                          <div style={{ fontSize:"11px", color:"#6B9A7A", marginTop:"2px" }}>FCFA</div>
                        </div>
                      </div>
                    )}
                    ...
                    {!histoAboLoading && histoAboErreur && (
                      <div style={{ background:"#FFF8E6", border:"1px solid #F0D58C", borderRadius:"10px", padding:"16px 20px", color:"#92700A", fontSize:"13px" }}>
                        <div style={{ fontWeight:700, marginBottom:"6px" }}> Route backend manquante</div>
                        <div style={{ lineHeight:1.6 }}>{histoAboErreur}</div>
                        <div style={{ marginTop:"12px", background:"#fff", borderRadius:"8px", padding:"10px 14px", fontSize:"12px", color:"#555", fontFamily:"monospace" }}>
                          // À ajouter dans routes/abonnements.js :<br/>
                          router.get("/historique", auth, async (req, res) =&gt; {"{"}<br/>
                          &nbsp;&nbsp;const data = await Abonnement.find({"{"} user: req.user._id {"}"}).sort({"{"} createdAt: -1 {"}"});<br/>
                    ...
                    {!histoAboLoading && !histoAboErreur && histoAbo.length > 0 && (
                      <div style={{ border:"1px solid #E2EDE6", borderRadius:"12px", overflow:"hidden" }}>
                        {/* En-tête du tableau */}
                        <div style={{ display:"grid", gridTemplateColumns:"1fr 120px 120px 100px", gap:"0", background:"#F5FAF7", borderBottom:"1px solid #E2EDE6", padding:"10px 16px" }}>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em" }}>Description</div>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", textAlign:"right" }}>Montant</div>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", textAlign:"right" }}>Solde après</div>
                          <div style={{ fontSize:"11px", fontWeight:700, color:"#6B9A7A", textTransform:"uppercase", letterSpacing:"0.06em", textAlign:"right" }}>Date</div>
                        </div>

                        {/* Lignes */}
                        {histoAbo.map((h, i) => {
                          const estRecharge = h.type === "recharge" || (h.montant > 0);
                          const montant     = h.montant || 0;
                          const date        = h.createdAt
                            ? new Date(h.createdAt).toLocaleDateString("fr-FR", { day:"2-digit", month:"short", year:"numeric" })
                            : h.date || "—";
                          const heure = h.createdAt
                            ? new Date(h.createdAt).toLocaleTimeString("fr-FR", { hour:"2-digit", minute:"2-digit" })
                            : "";

                          return (
                            <div key={h._id||i}
                              style={{ display:"grid", gridTemplateColumns:"1fr 120px 120px 100px", gap:"0",
                                padding:"12px 16px", borderBottom:i<histoAbo.length-1?"1px solid #F0F4F0":"none",
                              ...
                              <div>
                                <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                                  <div style={{ width:"28px", height:"28px", borderRadius:"50%", flexShrink:0,
                                    background:estRecharge?"#E8F5EE":"#FFF0F0",
                                    display:"flex", alignItems:"center", justifyContent:"center", fontSize:"13px" }}>
                                    {estRecharge ? "" : ""}
                                  </div>
                                  <div>
                                    <div style={{ fontWeight:600, fontSize:"13px", color:"#0A2410" }}>
                                      {h.description || (estRecharge ? "Recharge" : "Déduction")}
                                    </div>
                                    {h.reference && (
                                      <div style={{ fontSize:"10px", color:"#9AB0A0" }}>Réf. {h.reference}</div>
                                    )}
                                  </div>
                                </div>
                              </div>
                              ...
                              <div style={{ textAlign:"right" }}>
                                <span style={{ fontWeight:800, fontSize:"14px",
                                  color: estRecharge ? "#00904C" : "#E85555" }}>
                                  {estRecharge ? "+" : "-"}{Math.abs(montant).toLocaleString("fr-FR")}
                                </span>
                                <span style={{ fontSize:"10px", color:"#6B9A7A", marginLeft:"3px" }}>FCFA</span>
                              </div>

                              {/* Solde après */}
                              <div style={{ textAlign:"right" }}>
                                {h.soldeApres !== undefined ? (
                                  <span style={{ fontSize:"13px", fontWeight:600, color:"#0A2410" }}>
                                    {h.soldeApres.toLocaleString("fr-FR")} <span style={{ fontSize:"10px", color:"#6B9A7A" }}>FCFA</span>
                                  </span>
                                ) : <span style={{ color:"#9AB0A0", fontSize:"12px" }}>—</span>}
                              </div>

                              {/* Date */}
                              <div style={{ textAlign:"right" }}>
                                <div style={{ fontSize:"11px", color:"#0A2410", fontWeight:500 }}>{date}</div>
                                {heure && <div style={{ fontSize:"10px", color:"#9AB0A0" }}>{heure}</div>}
                              </div>
                            </div>
                          );
                        })}
                  ...
                  <div style={{ background:"var(--green-pale)", borderRadius:"12px", padding:"16px 20px", border:"1px solid var(--border)" }}>
                    ...
                    {!quota.illimite && (
                      <div style={{ height:"8px", borderRadius:"100px", background:"var(--border)", overflow:"hidden" }}>
                        <div style={{ height:"100%", borderRadius:"100px", width:`${Math.min(100,(quota.utilise/quota.quota)*100)}%`, background:quota.restant===0?"#FF6B6B":quota.restant<=5?"#D4A830":"#4DC97A" }}/>
                      </div>
                    )}
            ...
            {activeTab==="historique" && (
              <div className="profil-section">
                <div className="profil-section-header">
                  ...
                  <button className="btn-save" style={{ fontSize:"12px", padding:"8px 16px" }} onClick={()=>{ chargerHistoRecherches(); chargerHistoDemandes(); }}> Actualiser</button>
                </div>
                <div style={{ display:"flex", marginBottom:"24px", borderBottom:"2px solid var(--border)" }}>
                  {[{ key:"recherches", label:"Recherches", count:histoRecherches.length }, { key:"demandes", label:"Demandes", count:histoDemandes.length }].map(s=>(
                    <button key={s.key} onClick={()=>setSectionHisto(s.key)} style={{ padding:"10px 20px", background:"transparent", border:"none", borderBottom:sectionHisto===s.key?"3px solid var(--green-light)":"3px solid transparent", color:sectionHisto===s.key?"var(--green-dark)":"var(--text-muted)", fontWeight:sectionHisto===s.key?700:500, fontSize:"13px", cursor:"pointer", fontFamily:"inherit", marginBottom:"-2px", display:"flex", alignItems:"center", gap:"8px" }}>
                      {s.label}
                      <span style={{ background:sectionHisto===s.key?"var(--green-pale)":"rgba(0,0,0,0.06)", color:sectionHisto===s.key?"var(--green-dark)":"var(--text-muted)", borderRadius:"100px", padding:"1px 8px", fontSize:"11px", fontWeight:700 }}>{s.count}</span>
                    </button>
                  ))}
                </div>
                ...
                {sectionHisto==="recherches" && (
                  <div>
                    ...
                    {!histoRechercheLoading&&histoRechercheErreur&&<div style={{ background:"#FFF0F0", border:"1px solid #FFB3B3", borderRadius:"12px", padding:"20px", textAlign:"center", color:"#CC3333" }}><p style={{ margin:0 }}>{histoRechercheErreur}</p><button className="btn-save" style={{ marginTop:"12px", fontSize:"12px" }} onClick={chargerHistoRecherches}>Réessayer</button></div>}
                    {!histoRechercheLoading&&!histoRechercheErreur&&histoRecherches.length===0&&<div style={{ textAlign:"center", padding:"40px", color:"var(--text-muted)" }}><div style={{ fontSize:"40px", marginBottom:"12px" }}></div><p style={{ marginBottom:"16px" }}>Aucune recherche enregistrée.</p><button className="btn-save" onClick={()=>navigate("/rechercheacc")}>Faire une recherche</button></div>}
                        ...
                        {histoRecherches.map((h,i)=>(
                          <div key={h._id||i} className="histo-item">
                            <div style={{ flex:1 }}>
                              ...
                              {h.criteres&&<div style={{ display:"flex", flexWrap:"wrap", gap:"6px", marginTop:"6px" }}>{h.criteres.rccm&&<Chip label={`RCCM: ${h.criteres.rccm}`} color="#1E60CC"/>}{h.criteres.ifu&&<Chip label={`IFU: ${h.criteres.ifu}`} color="#1E60CC"/>}{h.criteres.raisonSociale&&<Chip label={`Raison: ${h.criteres.raisonSociale}`} color="#1E60CC"/>}</div>}
                              <div className="histo-date">{h.createdAt?new Date(h.createdAt).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"})+" à "+new Date(h.createdAt).toLocaleTimeString("fr-FR",{hour:"2-digit",minute:"2-digit"}):"—"} · <strong>{h.nbResultats??0} résultat{(h.nbResultats??0)>1?"s":""}</strong></div>
                              {replayMessage.id===h._id&&replayMessage.texte&&<div style={{ marginTop:"8px", padding:"8px 12px", borderRadius:"8px", fontSize:"12px", background:replayMessage.type==="succes"?"#E8F5EE":"#FFF0F0", color:replayMessage.type==="succes"?"#1A7A40":"#CC3333" }}>{replayMessage.texte}</div>}
                            </div>
                            <button className="btn-relancer" onClick={()=>relancerRecherche(h)} disabled={replayLoadingId===h._id} style={{ minWidth:"110px" }}>{replayLoadingId===h._id?"Vérification...":"↩ Relancer"}</button>
                          </div>
                        ))}
                      </div>
                    )}
                            ...
                            <div key={d._id||i} style={{ background:"#fff", borderRadius:"12px", border:"1px solid var(--border)", padding:"16px 20px" }}>
                              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"10px" }}>
                                <div>
                                  ...
                                  <div style={{ fontSize:"11px", color:"var(--text-muted)", marginTop:"3px" }}>{d.createdAt?new Date(d.createdAt).toLocaleDateString("fr-FR",{day:"2-digit",month:"long",year:"numeric"}):"—"} · Réf. <strong>{d._id?.slice(-6).toUpperCase()}</strong></div>
                                </div>
                              ...
                              {(d.regions?.length>0||d.activites?.length>0)&&<div style={{ display:"flex", flexWrap:"wrap", gap:"6px" }}>{d.regions?.map(r=><Chip key={r} label={r}/>)}{d.activites?.map(a=><Chip key={a} label={ACTIVITES.find(x=>x.value===a)?.label||a}/>)}</div>}
                            </div>
                          );
                        })}
                      </div>
            ...
            {activeTab==="securite" && (
              <div className="profil-section">
                ...
                <div className="security-item">
                  ...
                  <button onClick={deconnecterTout} disabled={deconnLoading} style={{ padding:"9px 18px", borderRadius:"8px", background:"#FFF0F0", border:"1.5px solid #FFB3B3", color:"#CC3333", fontWeight:700, fontSize:"13px", cursor:"pointer", fontFamily:"inherit" }}>{deconnLoading?"...":" Déconnecter tout"}</button>
                </div>
                <div style={{ marginTop:"24px", background:"#FFF5F5", border:"1px solid rgba(232,85,85,0.2)", borderRadius:"12px", padding:"18px 20px" }}>
                  <div style={{ fontWeight:700, fontSize:"14px", color:"#CC3333", marginBottom:"6px" }}> Zone dangereuse</div>
                  <div style={{ fontSize:"13px", color:"#6B9A7A", marginBottom:"14px" }}>La déconnexion de toutes les sessions fermera votre compte sur tous les appareils.</div>
                  <button onClick={deconnecterTout} disabled={deconnLoading} style={{ padding:"10px 20px", borderRadius:"8px", background:"#CC3333", color:"#fff", border:"none", fontWeight:700, fontSize:"13px", cursor:"pointer", fontFamily:"inherit" }}>{deconnLoading?"Déconnexion en cours...":" Déconnecter tous les appareils"}</button>
                </div>
              </div>
            )}
            ...
            {activeTab==="notifications" && (
              <div className="profil-section">
                <div className="profil-section-header">
                  ...
                  <button className="btn-save" style={{ fontSize:"12px", padding:"8px 16px" }} onClick={chargerNotifs}> Actualiser</button>
                </div>
                {notifsLoading ? <div style={{ textAlign:"center", padding:"40px", color:"var(--text-muted)" }}> Chargement...</div> : (
                  <div className="notif-list">
                    <div className="notif-item unread" style={{ borderLeft:"3px solid #00904C" }}>
                      <div className="notif-icon"></div>
                      <div style={{ flex:1 }}><div className="notif-msg">Bienvenue sur NERE CCI-BF !</div><div className="notif-date">Votre compte est actif et opérationnel.</div></div>
                    </div>
                    {user.emailVerified&&<div className="notif-item read"><div className="notif-icon"></div><div style={{ flex:1 }}><div className="notif-msg">Email vérifié</div><div className="notif-date">{user.email} — Adresse confirmée</div></div></div>}
                    {!rolePriv&&abonnement&&(
                      <div className="notif-item read">
                        <div className="notif-icon"></div>
                        <div style={{ flex:1 }}>
                          <div className="notif-msg">Solde disponible</div>
                          <div className="notif-date"><strong style={{ color:abonnement.solde<=0?"#FF3333":abonnement.solde<1000?"#D4A830":"#00904C" }}>{abonnement.solde?.toLocaleString("fr-FR")} FCFA</strong>{abonnement.solde<=0?" — Crédit épuisé, rechargez.":abonnement.solde<1000?" — Crédit faible, pensez à recharger.":" — Compte actif."}</div>
                        </div>
                    ...
                    {notifs.filter(n=>n.type==="diffusion").map(n=>(
                      <div key={n.id} className={`notif-item ${n.lu?"read":"unread"}`} style={{ borderLeft:!n.lu?"3px solid #00904C":"none" }}>
                        <div className="notif-icon"></div>
                        <div style={{ flex:1 }}><div className="notif-msg" style={{ fontWeight:!n.lu?700:500 }}>{n.msg}</div><div className="notif-date">{n.date}{n.heure&&` à ${n.heure}`}</div></div>
                    ...
                    {notifs.filter(n=>n.type==="diffusion").length===0&&<div style={{ textAlign:"center", padding:"24px", color:"var(--text-muted)", fontSize:"13px" }}><div style={{ fontSize:"32px", marginBottom:"8px" }}></div>Aucune annonce de la CCI-BF pour le moment</div>}
                    <div style={{ marginTop:"16px", padding:"16px", background:"var(--green-pale)", borderRadius:"12px", border:"1px solid var(--border)", display:"flex", alignItems:"center", gap:"12px" }}>
                      ...
                      <div style={{ flex:1 }}><div style={{ fontWeight:700, fontSize:"14px", color:"var(--text-dark)" }}>Messages directs</div><div style={{ fontSize:"12px", color:"var(--text-muted)" }}>Consultez vos échanges avec la CCI-BF</div></div>
                      <button onClick={()=>navigate("/chat")} style={{ padding:"8px 16px", borderRadius:"8px", background:"#00904C", border:"none", color:"#fff", fontWeight:700, fontSize:"13px", cursor:"pointer" }}>Ouvrir →</button>
                    </div>
                  </div>
                )}
              </div>
